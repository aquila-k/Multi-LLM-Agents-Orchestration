...
sync_context_from_brief() {
  ...
  sync_status=$(python3 - "$brief_output_file" "$context_file" <<'PYEOF'
import os
import re
import sys
...
sync_verify_commands_from_brief() {
  ...
  sync_status=$(python3 - "$brief_output_file" "$override_file" <<'PYEOF'
import os
import re
import sys
...
run_stage() {
  ...
  template_file="${PROMPTS_SRC}/${tool}/${role}.md"
  ...
  if [[ ! -f "$template_file" ]]; then
    ...
    log_error "Create prompts-src/${tool}/${role}.md before running this stage"
    exit 1
  fi
  ...
  if [[ $wrapper_exit -eq 0 ]]; then
    gate_output=$("${SCRIPT_DIR}/gate.sh" \
      --task "$TASK_DIR" \
      --stage "$stage" \
      --role "$role" 2>/dev/null) || gate_exit=$?
...
cmd_pipeline() {
  ...
  mapfile -t STAGE_PLAN < <(resolved_stage_plan)
