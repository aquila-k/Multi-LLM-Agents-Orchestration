...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --task-1-done) TASK_DONE[0]="$2"; shift 2 ;;
    --task-1-pid)  TASK_PID[0]="$2";  shift 2 ;;
    --task-1-out)  TASK_OUT[0]="$2";  shift 2 ;;
    --task-2-done) TASK_DONE[1]="$2"; shift 2 ;;
    --task-2-pid)  TASK_PID[1]="$2";  shift 2 ;;
    --task-2-out)  TASK_OUT[1]="$2";  shift 2 ;;
    --task-3-done) TASK_DONE[2]="$2"; shift 2 ;;
    --task-3-pid)  TASK_PID[2]="$2";  shift 2 ;;
    --task-3-out)  TASK_OUT[2]="$2";  shift 2 ;;
    --task-4-done) TASK_DONE[3]="$2"; shift 2 ;;
    --task-4-pid)  TASK_PID[3]="$2";  shift 2 ;;
    --task-4-out)  TASK_OUT[3]="$2";  shift 2 ;;
...
for (( i=0; i<TASK_COUNT; i++ )); do
  case "${TASK_STATUS[$i]}" in
    ...
    pending) FAIL_COUNT=$(( FAIL_COUNT + 1 )); log_error "[${LABEL}] task-$((i+1)): still pending (timed out)" ;;
  esac
done
