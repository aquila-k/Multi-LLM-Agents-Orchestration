...
Validates that collab/scripts/run works without any agent-collab/ dependency,
that standalone config loads correctly, and that PATH/preflight detection
works as expected.

Does NOT require live provider CLI execution.
Evidence is written to collab/facts/probe-results/check-entrypoint.json.
"""
...
def validate_entrypoint_help() -> dict:
    """Verify collab/scripts/run --help works."""
    ...
    return {
        ...
        "notes": "collab/scripts/run --help should exit 0 with usage info.",
    }
...
def validate_config_resolution(workspace_root: Path) -> dict:
    ...
    from collab.runtime.config_loader import load_config_bundle
    from collab.runtime.pathing import resolve_runtime_paths
    ...
    try:
        ...
        config_root_str = str(bundle.support_collab_root)
        return {
            "check": "config_resolution",
            "support_collab_root": config_root_str,
            ...
            "notes": "Config must load from collab/ only, never from agent-collab/.",
        }
...
def validate_preflight(workspace_root: Path) -> dict:
    ...
    sys.path.insert(0, str(REPO_ROOT))
    from collab.runtime.pathing import resolve_runtime_paths
    from collab.runtime.preflight import run_preflight
...
def run_check() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="collab-check-entrypoint-") as tmp:
        workspace = Path(tmp)
        checks = [
