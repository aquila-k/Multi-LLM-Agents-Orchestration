...
Evidence is written to collab/facts/probe-results/check-review-harden.json.
"""
...
def check_independent_review(workspace: Path) -> dict:
    ...
    try:
        ...
        task_id = entry["task_id"]
        task_artifacts = review_ws / "collab" / "artifacts" / "tasks" / task_id
...
def check_independent_harden(workspace: Path) -> dict:
    ...
    try:
        ...
        task_id = entry["task_id"]
        task_artifacts = harden_ws / "collab" / "artifacts" / "tasks" / task_id
...
def check_composed_review_harden(workspace: Path) -> dict:
    ...
    try:
        ...
        task_id = entry["task_id"]
        task_artifacts = composed_ws / "collab" / "artifacts" / "tasks" / task_id
...
def check_with_harden_entry_constraint() -> dict:
    ...
    sys.path.insert(0, str(REPO_ROOT))
    from collab.runtime.task_entrypoint import RequestValidationError, validate_request
