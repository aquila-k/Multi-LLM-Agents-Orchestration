...
Evidence is recorded in `collab/facts/probe-results/`:
...
Re-run `python3 collab/scripts/validation/run_checks.py` when any of the following change:

1. Provider adapter command-line flags (`collab/configs/providers/*.json`)
2. `collab/runtime/providers/*.py` execution logic
3. `collab/runtime/manifest_store.py` or `artifact_consistency.py` contracts
