...
from collab.runtime.task_entrypoint import RequestValidationError, run_task_from_request
...
class MalformedRequestRegressionTest(unittest.TestCase):
    def test_rejects_request_missing_required_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            task_root = root / "collab" / "artifacts" / "tasks" / "task-1"
            state_dir = root / "collab" / "state" / "tasks" / "task-1"
