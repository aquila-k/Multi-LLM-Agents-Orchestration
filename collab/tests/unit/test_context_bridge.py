...
class ContextBridgeUnitTest(unittest.TestCase):
    ...
    def test_load_task_context_handles_timeout(self) -> None:
        with patch("collab.runtime.context_bridge.subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="", timeout=0.1)):
    ...
    def test_get_current_revision_returns_snapshot_revision(self) -> None:
        result = get_current_revision(
            root_dir=Path("."),
            task_id="T6",
            runner=lambda *args, **kwargs: {"ok": True, "task_snapshot": {"revision": 7}},
        )
