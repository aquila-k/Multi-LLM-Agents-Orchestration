        ...
        try:
            ...
            if status in ("superseded", "archived"):
                ...
                _upsert_vector_projection(
                    ...
                    revision=row["revision"] if isinstance(row, sqlite3.Row) else row[4],
                    scope_ref=row["scope_ref"] if isinstance(row, sqlite3.Row) else row[3],
                    search_text_en="",
                    search_text_en_hash="",
                    canonical_json_hash="",
                    source_language="und",
    ...
    for item in pending_items:
        ...
        try:
            if item["needs_embed"] and item["search_text_en"].strip():
                ...
                if not embedding:
                    mark_failed(conn, queue_id, "embedding generation returned empty vector")
                    failed += 1
                    continue
