...
import agentorch_ctx.tests.integration.test_phase_runner as phase_runner_integration
...
_VALID_PLAN_CONTENT = "\n".join(
    [
        "1. Update agentorch_ctx/runtime/phase_runner.py with context write-back hooks",
...
class PhaseRunnerContextIntegrationTest(unittest.TestCase):
    ...
    def test_write_back_context_called_after_phase(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with patch(
                "agentorch_ctx.runtime.phase_runner._write_back_context"
            ) as write_back:
    ...
    def test_context_bridge_failures_are_degraded(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with patch(
                "agentorch_ctx.runtime.context_bridge.save_task_snapshot",
                side_effect=RuntimeError("boom"),
            ):
                with patch(
                    "agentorch_ctx.runtime.context_bridge.log_episode",
                    side_effect=RuntimeError("boom"),
                ):
                    with patch(
                        "agentorch_ctx.runtime.context_bridge.log_decision",
                        side_effect=RuntimeError("boom"),
                    ):
    ...
    def test_plan_phase_logs_decisions_for_findings(self) -> None:
        ...
        self.assertFalse(
            any(
                "Extra question"
                in str(call["kwargs"].get("payload", {}).get("summary", ""))
                for call in recorded_decisions
            )
        )
