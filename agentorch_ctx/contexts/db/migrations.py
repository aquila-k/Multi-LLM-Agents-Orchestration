...
def get_sql_dir(runtime_root: Path) -> Path:
    """Returns path to the sql/ directory containing migration files.

    Resolution order:
      1. ``<runtime_root>/sql/`` — when runtime_root is the contexts package
         directory itself (e.g. ``agentorch_ctx/contexts/``)
      2. ``<runtime_root>/../sql/`` — legacy layout where runtime_root is
         ``.contexts/runtime/`` and sql lives at ``.contexts/sql/``
    """
    pkg_sql = runtime_root / "sql"
    if pkg_sql.is_dir():
        return pkg_sql
