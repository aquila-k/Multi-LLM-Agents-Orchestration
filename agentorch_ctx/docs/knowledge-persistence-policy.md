...
Define how the standalone agentorch runtime persists task state, known
...
Mutable task state lives under `.agentorch/state/tasks/<task_id>/`.
...
Append-only execution artifacts live under `.agentorch/artifacts/tasks/<task_id>/`.
...
2. Controllers such as `Claude Code` may read summaries and stop reasons, but
   do not redefine agentorch persistence semantics.
