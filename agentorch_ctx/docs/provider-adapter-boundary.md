...
Define what provider adapters may own inside the standalone agentorch runtime
...
1. `agentorch_ctx/runtime/providers/__init__.py`
2. `agentorch_ctx/runtime/providers/base.py`
3. `agentorch_ctx/runtime/providers/codex.py`
4. `agentorch_ctx/runtime/providers/gemini.py`
5. `agentorch_ctx/runtime/providers/copilot.py`
6. `agentorch_ctx/runtime/runtime_coordinator.py`

Live-validation corrections should first be written back to:

1. `agentorch_ctx/configs/providers/*.json` for command, model, timeout, or retry defaults
2. `agentorch_ctx/facts/verified-facts/*.json` for promoted or corrected capability evidence
3. `agentorch_ctx/facts/validation-debt.json` for unresolved provider claims
4. `agentorch_ctx/docs/release-gate-checklist.md` and `V10` for procedure updates
